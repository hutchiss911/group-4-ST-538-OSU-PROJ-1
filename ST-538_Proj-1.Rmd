...
regfit.best <- regsubsets(Income ~ ., data = puma_data_q2[train, ], nvmax = 19)
...
#perform best subset selection on the full data set, and select the best 19 variable model
regfit.best <- regsubsets(Income ~ ., data = puma_data_q2, nvmax = 19)
coef(regfit.best, 19)
```
...
```
Cross-validation selects a 19-variable model. We now perform best subset selection on the full data set in order to obtain the 19-variable model.

```{r}
reg.best <- regsubsets(Income ~ ., data = puma_data_q2, nvmax = 19)
coef(reg.best, 19)
```

```{r}
#TRY Best subset selection
regfit.full <- regsubsets(Income ~ ., puma_data_q2) 
summary(regfit.full)

unique(puma_data_q2$OccupationGroup)
```
