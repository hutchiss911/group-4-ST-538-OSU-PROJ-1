...
#Make sure it is numeric
puma_data$EducationLevel = as.numeric(puma_data$EducationLevel)
...
Next we'll take a look into the income distribution in a graphical view. For this view we will also remove those that are below 0. This allows us to see a very long tailed view of the income distribution for our dataset. Our median being the tallest point in the graph being around that $33,700 we saw in our exploration.
...
We compared two regression models, the first model has EducationGroup, Sex, and Age as explanatory variables. The second model has EducationGroup, Sex, Age and interaction between Sex and Age. After conducting ANOVA comparisons between the full and nested models, there was compelling evidence that the second model, which included an interaction term between Sex and and Age was a better fit. However, further examination of the model diagnostics revealed a violation of the linearity assumption. To address this issue, we transformed the response variable using logarithm.
...
p1 <- qplot(ID,.hat, data = Data)
p2 <- qplot(ID,.stdresid, data = Data)
p3 <- qplot(ID,.cooksd, data = Data)
...
plot(model_4, which = 4, cook.levels = cutoff)
...
$$log(income_i) = 9.257 +  0.554(HS, GED, or Associates Degree) + 1.193(Bachelors Degree) + 1.527(Masters Degree or higher) + 0.255(Male) + 0.003(Age) + 0.003(Male:Age)$$
```{r}
unique(puma_data$EducationGroup)
```
...
To answer our question, does higher education mean higher income accounting for sex and age? Use the fitted model to compare the income across all 4 levels of education for a 50 years old males and females in Oregon.

```{r, message=FALSE, echo=FALSE, results='hide', warning=FALSE}
#Use predict() to fit the regression and find the average income
new_data_1 <- data.frame(EducationGroup = c("Less than HS","HS, GED, or Associates Degree", "Bachelors Degree", "Masters Degree or higher"),
                       Sex = "Male",
                       Age = 50)

mu_1 <- predict(model_4, newdata = new_data_1 , type = "response")
exp(mu_1)
```
...
```{r, message=FALSE, echo=FALSE, results='hide', warning=FALSE}
#Use predict() to fit the regression and find the average income
new_data_2 <- data.frame(EducationGroup = c("Less than HS","HS, GED, or Associates Degree", "Bachelors Degree", "Masters Degree or higher"),
                       Sex = "Female",
                       Age = 50)

mu_2 <- predict(model_4, newdata = new_data_2 , type = "response")
exp(mu_2)
```
